...
const getTransporter = async () => {
  const configs = await query('SELECT chave, valor FROM emailpericia.configuracoes WHERE chave LIKE \'SMTP_%\'');
...
const sendEmail = async (to, subject, html, attachments = []) => {
  ...
  const configs = await query('SELECT valor FROM emailpericia.configuracoes WHERE chave = \'SMTP_FROM_NAME\' OR chave = \'SMTP_FROM_EMAIL\'');
